...
### Outdoor Unit Silent Mode

Outdoor Unit Silent Mode is a model-dependent protocol finding ported from
`hberntsen/mhi-ac-ctrl-esp32` commit `538b3412b4e013d6f732f47736f8446a211ec737`.
It is distinct from the indoor fan Quiet level.

The controller polls Silent Mode with:
...
A Silent Mode feedback frame is identified by:
...
The state is carried in `DB11`:

```text
DB11 & 0x20 != 0  -> Silent Mode enabled
DB11 & 0x20 == 0  -> Silent Mode disabled
```

The dedicated write form is:

```text
MISO DB6  = 0x80
MISO DB9  = 0x21
MISO DB10 = 0x01  -> enable
MISO DB10 = 0x00  -> disable
```

The `DB10=0x80` feedback form does not belong to the normal `0x10`, `0x20`, or
`0x30` opdata response families, so it requires explicit classification. Feedback
may arrive independently of the normal status-frame phase and is accepted on
either phase.

The ESPHome switch is deliberately opt-in. Enabling the entity adds the
`0xC0/0xDD` request to the opdata scheduler. Until a valid response is observed,
the feature remains unknown rather than fabricating an `OFF` state. A write is
confirmed only when returned opdata reports the requested state.

This mapping is not yet claimed as universal across all MHI models. Hardware
validation should record whether the unit responds to the poll, accepts both
write values, and returns authoritative feedback after each command.
...
- Outdoor Unit Silent Mode confirms from decoded `0xDD/0x80` operation-data feedback;
