...
void diagnostics_snapshot_reports_rx_worker_timing() {
  ...
  diagnostics.stats().on_rx_worker_timing(50000U, 1000U);
  diagnostics.stats().on_rx_worker_frame_queued(1U, 1000U);
  diagnostics.stats().on_rx_worker_timing(48000U, 2000U);
  diagnostics.stats().on_rx_worker_frame_queued(2U, 2000U);
  diagnostics.stats().on_rx_worker_frame_drained(1U, 2500U);
  diagnostics.stats().on_rx_worker_queue_overflow(4U, 2600U);

  const auto snapshot = diagnostics.snapshot(3000U);

  EXPECT_EQ(snapshot.stats.rx_worker_samples, 2U);
  EXPECT_EQ(snapshot.stats.rx_worker_frames, 2U);
  EXPECT_EQ(snapshot.stats.rx_worker_drained_frames, 1U);
  EXPECT_EQ(snapshot.stats.rx_worker_queue_overflows, 1U);
  EXPECT_EQ(snapshot.stats.queue_overflows, 1U);
  EXPECT_EQ(snapshot.stats.rx_worker_queue_depth, 4U);
  EXPECT_EQ(snapshot.stats.rx_worker_queue_max_depth, 4U);

  EXPECT_EQ(snapshot.stats.rx_worker_last_us, 48000U);
  EXPECT_EQ(snapshot.stats.rx_worker_avg_us, 49000U);
  EXPECT_EQ(snapshot.stats.rx_worker_max_us, 50000U);

  EXPECT_EQ(snapshot.last_rx_worker_frame_age_ms, 1000U);
  EXPECT_EQ(snapshot.last_rx_worker_drained_frame_age_ms, 500U);
  EXPECT_EQ(snapshot.last_rx_worker_queue_overflow_age_ms, 400U);
}

void diagnostics_snapshot_reports_rx_worker_health_counters() {
  MhiDiagnostics diagnostics{};

  diagnostics.stats().on_rx_worker_health_check(true, false, false, 1000U);
  diagnostics.stats().on_rx_worker_health_check(false, true, true, 2000U);

  const auto snapshot = diagnostics.snapshot(2500U);

  EXPECT_EQ(snapshot.stats.rx_worker_health_checks, 2U);
  EXPECT_EQ(snapshot.stats.rx_worker_no_frame_windows, 1U);
  EXPECT_EQ(snapshot.stats.rx_worker_stalls, 1U);
  EXPECT_EQ(snapshot.stats.rx_worker_not_draining_windows, 1U);
  EXPECT_EQ(snapshot.last_rx_worker_no_frame_window_age_ms, 1500U);
  EXPECT_EQ(snapshot.last_rx_worker_stall_age_ms, 500U);
  EXPECT_EQ(snapshot.last_rx_worker_not_draining_age_ms, 500U);
}
